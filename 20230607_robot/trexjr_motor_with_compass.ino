...
const byte rxPinsw = 11;
const byte txPinsw = 12;
SoftwareSerial mySerialMotor = SoftwareSerial(rxPin, txPin);
SoftwareSerial mySerialData = SoftwareSerial(rxPinsw,txPinsw);
...
void setup() {
  ...
  pinMode(rxPinsw,INPUT);
  pinMode(txPinsw,OUTPUT);
  Serial.begin(9600);
  
  mySerialMotor.listen();
  mySerialMotor.begin(19200);
  

  mySerialData.begin(57600);
  ...
  checkSettings();
  Serial.println("finished with compass initialization");
  ...
  Serial.println(x_norm);
    ...
    while ((ikount < 1000 ) ) {
      ...
      mySerialData.listen();
      for (iloop = 0; iloop < 5; iloop = iloop + 1)
        {
...
void checkSettings()
{
  ...
  Serial.println("at end of check settings");
}
